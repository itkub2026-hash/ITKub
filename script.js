...
function closeCabinetModalOutside(e) {
  if (e.target.id === 'cabinet-modal-overlay') {
    document.getElementById('cabinet-modal-overlay').classList.add('hidden');
    history.back();
  }
}

function closeTeamsModalOutside(e) {
  if (e.target.id === 'teams-modal-overlay') {
    document.getElementById('teams-modal-overlay').classList.add('hidden');
  }
}

function hideAllAuth() {
  ['auth-login','auth-register','auth-verify','auth-forgot'].forEach(id => {
    document.getElementById(id).classList.add('hidden');
  });
  ['login-error','register-error','verify-error','forgot-error'].forEach(id => {
    document.getElementById(id).classList.add('hidden');
  });
}
...
function showForgotPassword() {
  ...
  document.getElementById('forgot-verify').classList.add('hidden');
}
...
async function confirmRegistration() {
  ...
  const response = await fetch(`${VERCEL_URL}/api/verify-code`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: pendingRegData.email, code: code })
  });
  const data = await response.json();
  ...
  currentUser = {
    firstName: pendingRegData.firstName,
    lastName: pendingRegData.lastName,
...
async function resetPassword() {
  ...
  const vResp = await fetch(`${VERCEL_URL}/api/verify-code`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  await fetch(`${VERCEL_URL}/api/user`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ action: 'reset-password', email: email, password: newPassword })
  });
...
function showConfirmModal(title, message, onConfirm) {
  ...
  overlay.onclick = function(e) {
    if (e.target.id === 'custom-confirm-overlay') overlay.remove();
  };
  overlay.innerHTML = `
    <div class="modal-content p-8 text-center" style="max-width: 420px;" onclick="event.stopPropagation()">
      ...
      <div class="flex gap-3">
        <button id="custom-confirm-cancel" class="flex-1 bg-gray-700 hover:bg-gray-600 py-4 rounded-2xl transition">Отмена</button>
        <button id="custom-confirm-ok" class="flex-1 bg-gradient-to-r from-red-500 to-red-600 hover:scale-105 py-4 rounded-2xl transition font-semibold">Подтвердить</button>
      </div>
    </div>
  `;
...
async function showMyTeams() {
  ...
  list.innerHTML = data.teams.map(t => {
    const memberList = t.members.map(m => {
      return `<span class="text-gray-400">${m.name} <span class="text-xs text-gray-500">(${m.role || 'Участник'})</span></span>`;
    }).join(', ');

    const isCaptain = t.captain === currentUser.email;
      ...
      <div class="bg-gray-800 rounded-2xl p-4">
        <div class="flex justify-between items-start">
          <div>
            ...
            <div class="text-gray-500 text-xs mt-1">${memberList}</div>
          </div>
          <div class="flex flex-col gap-2 items-end">
            ...
            ` : `
              <button onclick="leaveTeam('${t.inviteCode}')" class="text-gray-400 hover:text-white text-sm"><i class="fas fa-sign-out-alt"></i></button>
            `}
            <button onclick="changeMyRole('${t.inviteCode}')" class="text-yellow-400 hover:text-yellow-300 text-sm"><i class="fas fa-tag"></i></button>
          </div>
        </div>
        ${isCaptain ? `
          <div class="mt-3 pt-3 border-t border-gray-700">
            <p class="text-gray-400 text-xs mb-2">Управление участниками:</p>
            ${t.members.filter(m => m.email !== currentUser.email).map(m => `
              <div class="flex justify-between items-center py-1">
                <span class="text-sm text-white">${m.name}</span>
                <button onclick="kickMember('${t.inviteCode}','${m.email}','${m.name}')" class="text-red-400 text-xs hover:underline"><i class="fas fa-times"></i></button>
              </div>
            `).join('') || '<p class="text-gray-600 text-xs">Нет других участников</p>'}
          </div>
        ` : ''}
      </div>
    `;
...
function showJoinTeam() {
  ...
  document.getElementById('join-role').value = 'Участник';
}

async function joinTeam() {
  const code = document.getElementById('join-invite-code').value.toUpperCase().trim();
  const role = document.getElementById('join-role').value.trim() || 'Участник';
...
async function changeMyRole(inviteCode) {
  const resp = await fetch(`${VERCEL_URL}/api/teams?email=${encodeURIComponent(currentUser.email)}`);
  const data = await resp.json();
  const team = data.teams?.find(t => t.inviteCode === inviteCode);
  const member = team?.members?.find(m => m.email === currentUser.email);
  const currentRole = member?.role || 'Участник';

  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
  overlay.id = 'change-role-overlay';
  overlay.onclick = function(e) {
    if (e.target.id === 'change-role-overlay') overlay.remove();
  };
  overlay.innerHTML = `
    <div class="modal-content p-8" style="max-width: 400px;" onclick="event.stopPropagation()">
      <h3 class="text-2xl font-bold mb-4 text-center">Смена роли</h3>
      <p class="text-gray-400 text-sm mb-2">Текущая роль: <span class="text-white">${currentRole}</span></p>
      <input type="text" id="new-role-input" value="${currentRole}" placeholder="Новая роль" class="w-full bg-gray-800 border border-gray-700 rounded-2xl px-6 py-4 mb-6">
      <div class="flex gap-3">
        <button onclick="document.getElementById('change-role-overlay').remove()" class="flex-1 bg-gray-700 hover:bg-gray-600 py-4 rounded-2xl transition">Отмена</button>
        <button onclick="saveNewRole('${inviteCode}')" class="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 hover:scale-105 py-4 rounded-2xl transition font-semibold">Сохранить</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
...
async function saveNewRole(inviteCode) {
  const role = document.getElementById('new-role-input').value.trim();
  if (!role) return showToast('Введите роль', true);

  const resp = await fetch(`${VERCEL_URL}/api/teams`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      action: 'change-role',
  ...
  const data = await resp.json();
  if (data.success) {
    document.getElementById('change-role-overlay').remove();
...
async function kickMember(inviteCode, targetEmail, targetName) {
  showConfirmModal(
    'Исключить участника',
    `Вы уверены, что хотите исключить ${targetName} из команды?`,
    async function() {
      const resp = await fetch(`${VERCEL_URL}/api/teams`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function leaveTeam(inviteCode) {
  showConfirmModal(
    'Выйти из команды',
...
async function editTeam(inviteCode) {
  ...
  overlay.onclick = function(e) {
    if (e.target.id === 'edit-team-overlay') overlay.remove();
  };
  overlay.innerHTML = `
...
async function saveTeamChanges(inviteCode) {
  ...
  if (!caseNum || isNaN(caseNum) || caseNum < 1 || caseNum > 4) {
    return showToast('Номер кейса от 1 до 4', true);
  }
...
async function disbandTeam(inviteCode) {
  showConfirmModal(
    ...
    'Вы уверены, что хотите полностью расформировать команду? Это действие нельзя отменить.',
    async function() {
      const respTeams = await fetch(`${VERCEL_URL}/api/teams?email=${encodeURIComponent(currentUser.email)}`);
      const dataTeams = await respTeams.json();
      const team = dataTeams.teams?.find(t => t.inviteCode === inviteCode);
  ...
  try {
    ...
    if (data.events && data.events.length > 0) {
      ...
      for (const event of events) {
        event._savedStatus = event.status?.text || null;
      }
    }
  } catch (e) {
...
function updateEventUI(event) {
  ...
  // Админ явно закрыл (статус в базе "закрыта", но места есть)
  if (event._savedStatus === 'Регистрация закрыта' && !isFull) {
...
function renderEvents() {
  ...
  if (!events || events.length === 0) {
    container.innerHTML = `
      <div class="col-span-full text-center py-20">
        ...
        <h2 class="text-2xl font-bold text-gray-400 mb-2">Мероприятия скоро появятся</h2>
        <p class="text-gray-500">Актуальные мероприятия сейчас добавляются. Загляните позже!</p>
      </div>
    `;
    return;
  }
  ...
  events.forEach(function(event) {
    updateEventUI(event);
    const isClosed = event.status && event.status.text === 'Регистрация закрыта';
...
function renderDetailPage() {
    ...
    <div class="p-8 md:p-12">
      ...
      <div class="mt-16 bg-gray-800 rounded-3xl p-10">
        ...
        <form id="reg-form" class="space-y-6 max-w-lg mx-auto" novalidate>
          ...
          <div>
            <input type="text" id="captain-role" placeholder="Ваша роль (например: Frontend-разработчик)" class="w-full bg-gray-900 border border-gray-700 rounded-2xl px-6 py-4">
          </div>
          <button type="submit" ${isClosed || !currentUser ? 'disabled' : ''} class="w-full ${isClosed || !currentUser ? 'bg-red-600/80 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:scale-105'} py-5 rounded-3xl text-lg font-semibold transition">
            ${!currentUser ? 'Войдите для регистрации' : isClosed ? 'Регистрация закрыта' : 'Зарегистрировать команду'}
          </button>
        </form>
      </div>
    </div>
  ...
  if (form) {
    ...
    ['team-name', 'case-number', 'captain-role'].forEach(function(id) {
    ...
    newForm.addEventListener('submit', async function(e) {
      ...
      if (!currentUser.email) { showToast('Ошибка профиля. Войдите заново.', true); return; }
      if (isClosed) { showToast("Регистрация закрыта", true); return; }

      const teamName = document.getElementById('team-name').value.trim();
      const caseNum = parseInt(document.getElementById('case-number').value);
      const captainRole = document.getElementById('captain-role').value.trim() || 'Капитан';
      let isValid = true;

      if (!teamName) {
        showFieldError('team-name', 'Введите название');
        isValid = false;
      } else {
        clearFieldError('team-name');
      }

      if (!caseNum || isNaN(caseNum) || caseNum < 1 || caseNum > currentEvent.cases.length) {
        showFieldError('case-number', 'Номер кейса 1-' + currentEvent.cases.length);
        isValid = false;
      } else if (currentEvent.caseFree[caseNum - 1] <= 0) {
        showFieldError('case-number', 'Места закончились');
        isValid = false;
      } else {
        clearFieldError('case-number');
      }
      ...
      const checkData = await checkResp.json();
      if (checkData.teams && checkData.teams.some(t => t.eventTitle === currentEvent.title)) {
      ...
      showCodeModal(async function() {
        try {
          ...
          if (ps) {
            ...
            ps.className = currentEvent.freePlaces >= currentEvent.totalPlaces
              ? 'text-red-400'
              : 'text-emerald-400';
          }
